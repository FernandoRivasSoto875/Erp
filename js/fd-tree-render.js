(function(){
  ...
  function getGroupTitle(group){
    ...
    return group.getAttribute('data-fieldset-name') || group.id || 'Grupo';
  }
  function getGroupId(g){
    return g.getAttribute('data-group-id') || g.getAttribute('data-fieldset-name') || g.id || getGroupTitle(g).toLowerCase().replace(/\s+/g,'-');
  }
  ...
  function getFieldLabel(w){
    ...
    const nm = w.querySelector?.('[name]')?.getAttribute('name');
    return nm || 'Campo';
  }
  ...
  function buildTree(){
    ...
    if (!groups.length){
      panel.innerHTML = '<div class="text-muted small">No hay grupos que mostrar.</div>';
      return;
    }
